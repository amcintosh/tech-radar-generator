...
const Radar = function (size, radar) {
  ...
  const tip = d3.tip().attr('class', 'd3-tip').html(function (text) {
    return text
  })
  ...
  function constructSheetUrl (sheetName) {
    ...
    const sheetUrl = noParamUrl + '?sheetId=' + queryParams.sheetId + '&sheetName=' + encodeURIComponent(sheetName)
    return sheetUrl
  }
  ...
  self.plot = function () {
    let rings, quadrants, alternatives, currentSheet

    rings = radar.rings()
    quadrants = radar.quadrants()
    alternatives = radar.getAlternatives()
    currentSheet = radar.getCurrentSheet()
    const header = plotRadarHeader()
